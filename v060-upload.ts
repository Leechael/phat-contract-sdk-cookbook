...
import { signAndSend, signCertificate, OnChainRegistry, options, PinkCodePromise, KeyringPairProvider, type FrameSystemAccountInfo, PinkContractPromise, PinkContractQuery, PinkContractTx } from '@phala/sdk'
...
async function main() {
  ...
  const accountInfo = await api.query.system.account<FrameSystemAccountInfo>(user.address)
  const free = accountInfo.data.free.div(new BN(1e10)).toNumber() / 100
  if (free < 20) {
  ...
  console.log('Cluster Balance:', balance.total.toNumber() / 1e12, balance.free.toNumber() / 1e12)

  if ((balance.free.toNumber() / 1e12) < 500) {
  ...
  const contract = instantiateResult.contract as PinkContractPromise<
    {
      getTotalBadges: PinkContractQuery<[], [u64]>
    },
    {
      newBadge: PinkContractTx<[string]>
    }
  >
  ...
  //
  contract.provider = new KeyringPairProvider(api, user)

  //
  // query test. 
  //
  const old_call = await contract.query.getTotalBadges(user.address, { cert })
  const new_call = await contract.q.getTotalBadges<u64>()
